...
- **a comparação com incidência continua indireta, e exige um passo que este artigo não deu.** A coluna do padrão mundial (Tabela 2) usa a população da OMS 2000–2025, e as publicações do GLOBOCAN e do *Cancer Incidence in Five Continents* usam Segi–Doll; confrontá-las exige recalcular a série sob o padrão do comparador, e não apenas dispor de uma coluna padronizada (§2.2). Há ainda uma dependência que desaconselha ler a concordância como validação externa: a Estimativa do INCA usa registros de incidência **e** mortalidade, e onde a incidência observada é insuficiente a mortalidade entra na estimação — de modo que ela e este trabalho não são fontes independentes. Uma razão mortalidade/incidência construída assim ordenaria unidades da federação por um quociente cujo numerador e denominador se tocam;
